...
- success_criteria:
  ...
  - SourceReference supports provider, uri, revision, observed_at, and optional line/char/page/block spans without assuming every source is a local file.
  - The local file source provider emits provider=local_files references; a git history fixture can be represented with provider=git_history and commit-sha revision even if full history ingestion is deferred.
  - JSON Schema export matches committed golden file byte-for-byte after canonical formatting.
- description: |
    Define and implement the canonical data contracts for the system: the Claim record schema (stable id, claim text, provider-aware source references, source type, observed timestamp, confidence in [0,1], status enum {proposed, verified, conflict, stale, ruled_out, needs_human}, related_systems, related_owners), the SourceReference schema, and the artifact manifest schema describing the set of output files (claims.jsonl, ORG_CONTEXT.md, SYSTEMS.md, DECISIONS.md, OWNERS.md, GLOSSARY.md, CONTEXT_GAPS.md, RULEDOUT.md). SourceReference must include provider (local_files, git_history, github, gdrive, notion, other), uri, optional revision, optional line/char/page/block span fields, observed_at, and source_id; local files should use file URIs or normalized relative paths in uri rather than a file-only top-level path field. Provide a Python library module `context_formalizer.schemas` exposing pydantic (or dataclass + jsonschema) models, JSON Schema export, stable-id generation (content-addressed hash over normalized claim text + primary source provider/uri/revision/span), and round-trip (de)serialization helpers for JSONL. Include golden fixtures for each schema and a documented version field for forward migration.
...
- success_criteria:
  ...
  - When the root is a git repository, the default crawl uses `git ls-files` so ignored screenshots, caches, and build artifacts are excluded unless an explicit --include-untracked flag is provided.
  ...
  - A provider abstraction exists with a LocalFileSourceProvider implementation; connector/provider interfaces are documented but non-local providers are not implemented in the MVP.
...
- description: |
    Build a deterministic local-first crawler CLI subcommand `context-formalizer crawl --root <path>` backed by a SourceProvider interface and a LocalFileSourceProvider implementation. For a git repository root, use `git ls-files` by default; otherwise walk a bounded corpus while respecting an explicit allowlist of extensions and a .contextignore file. Classify each source by source type (markdown_doc, source_code, config, adr, issue_export, meeting_notes, git_commit, other), capture content hash, size, mtime, provider, uri, optional revision, and compute a stable source_id. It must not follow symlinks outside the root, must skip binary files above a size threshold, and must emit `sources.jsonl` plus a human-readable `INVENTORY.md`. No network access. The crawler is explicitly local-files-only in the MVP; no Slack/Drive/Notion/GitHub API/browser automation. A future GitHistoryProvider should be possible without changing Claim or SourceReference schemas.

### F3: Claim Candidate Extraction with Provenance
- task_type: library
- verifier: python_pytest
- success_criteria:
  - Every emitted claim has a non-empty source_references list with provider, uri, observed_at, and a resolvable local file span for local_files references (provenance invariant).
